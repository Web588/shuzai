      ...
      <div class="source-content">
        <ul>
          <li
            ...
            class="source-item"
            @click.stop="_changeSource(item,index)"
            :key="index"
          >
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    next(vm => {
      ...
      vm.getBookSources(vm.$route.params.id,vm.type);
      vm.getBook(vm.$route.params.id,vm.type);
    });
  },
  destroyed() {
    this.deleteTryRead();
  ...
  methods: {
    //获取是否为新用户，新用户的则需要引导页
    getNewUser() {
      let localShelf = JSON.parse(window.localStorage.getItem("bootPage"))
        ? JSON.parse(window.localStorage.getItem("bootPage"))
        : {};
    ...
    //删除试阅读记录
    deleteTryRead() {
      let localShelf = JSON.parse(window.localStorage.getItem("tBook"))
        ? JSON.parse(window.localStorage.getItem("tBook"))
        : {};
